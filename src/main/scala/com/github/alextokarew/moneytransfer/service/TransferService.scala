...
class TransferServiceImpl(
  ...
  override def createTransfer(request: TransferRequest): Valid[Transfer] = {
    ...
    validate(request)(
      ...
      check(_.amount > 0, "An amount to transfer must be strictly positive")).map { validRequest =>
  ...
  override def getTransfer(id: Long): Valid[Transfer] = validate(transferStorage.get(id))(
    check(_.isDefined, s"Transfer with id $id does not exist")).map(_.get)
}
...
object TransferServiceImpl {
  def apply(
    ...
    transferStorage: Storage[Long, Transfer],
    clock: Clock,
    lastId: Long = 0L): TransferServiceImpl = new TransferServiceImpl(processor, accountStorage, tokenStorage, transferStorage, clock, lastId)
}